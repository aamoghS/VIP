...
const questionBank = [
  ...
  // Science - Water Cycle/Rock Types
...
export async function GET(request: Request) {
  ...
  // Don't leak the exact answer text to the client immediately if we want to evaluate server-side later,
  // but since we want the client to evaluate instantly per prompt: "Individual/group live time if they are right or wrong"
  // we'll send it or keep it hidden depending on design. We'll send it for simplicity of the SSR frontend check.
